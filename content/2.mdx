...
A brief about myself: I'm a web developer, and **_ReactJS_** is my strong. In addition, I've used **_React Native_** (a cross-platform library which also use JS) for building several native apps. Then, I heard about Flutter, and its impressive improvement in 2019. And this post was written when I was starting to learn **Flutter**.
In this post, I will try to implement a horizontal list of a very popular mobile app: _Facebook Messenger_. I will try all my best to make this widget has flexible height, that means it can be reused in many layouts. And if you are getting to know **Flutter** (like me), I believe that this post will give you a few experiences.
...
Of course, the first thing that you need is creating a new Flutter project. If you don't know how to do that, **Flutter** had a very detail guide about this: https://flutter.dev/docs/get-started/test-drive

## 2. Create model, mock data

Create _user_model.dart_ to store User model:
...
Then, we will create _data.dart_ file to store our mock data:
...
Okay, we've done the preparing for _Model_ and _Data_.

## 3. Create Users List widget

Now, we are going to do the most important (and also the most difficult) step in this tutorial: implement **_Users List_** and **_User Item_** widget.
First, we will create _users_list.dart_, then create **_Users List_** widget:
...
Then, we will move to the implement of **_User Item_** widget:
...
As we can see, there are several part in an **_User item_**: <u>_Rounded Avatar Image_</u>, <u>_User's Name_</u>, <u>_Story Circle_</u>, and _<u>Active Indicator</u>_. So, we will do each part one by one in order. And <u>_avatar_</u> is the first one:

- Add an square image by using _Aspect Ratio_:
...
- Then, make the images rounded by adding _Material_ widget with _MaterialType.circle_:
...
- We will have this result. And that also means _Rounded Avatar Image_ has been done.
...
- Actually, _CircularProgressIndicator_ is the progress circle, but I can you it as a circle here with a little trick as you can see. If value is null, that widget will be in loading state with fancy animation.
...
- Finally, this is our final result:
...
In conclusion, although this is just a simple widget, we still get some experiences: how to manage the dimension of widget, how to use stack, the think to make the flexible widget, ... Anyway, the tutorial just carries my own opinion and thoughts, and I'm just a new guy in **Flutter**, so if there is any mistakes in my post, please comment and let me know. I would love to receive any comments whether positive or negative. That is also great motivation for me to have the next sharing. Many thanks.

P/S: as mentioned below, I'm a web developer, _React_ and _Javascript_ are my cores. So, I will have a post to compare between **React-Native** and **Flutter**, and share my difficulties when starting to learn Flutter as a JS dev.
